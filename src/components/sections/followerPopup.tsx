      ...
      <div className="bg-zinc-600 p-6 rounded-lg max-w-md w-full flex flex-col">
        ...
        <ul className="space-y-2">
          {paginatedFollowers.map((user) => (
            <li key={user.id} className="flex items-center space-x-4 cursor-pointer" onClick={() => handleFollowerClick(user.id)}>
        ...
        <div className="flex justify-between mt-4">
          ...
          <button className="px-4 py-2 rounded bg-gray-700 text-white disabled:opacity-50" disabled={endIndex >= followers.length} onClick={() => setPage(page + 1)}>
            Next
          </button>
        </div>
      </div>
